...
As the user, I understand that changing Carpeta de salida does not relocate old meetings, and that transcribing a file that already lives elsewhere still writes beside that file.

**Why this priority**: Prevents “helpfully” moving or duplicating archives; keeps import behavior from feature 009.

**Independent Test**: Leave an old meeting in folder A; change to B; transcribe the old file (or an import from another disk). Confirm A’s file is not copied into B; new meetings still go to B.

**Acceptance Scenarios**:

1. **Given** a finished video still in folder A, **When** the user switches Carpeta de salida to B and transcribes that existing A file, **Then** user-visible transcript output stays next to that A file; the original is not copied into B.
2. **Given** the user imports a podcast from a folder that is not B, **When** transcription finishes, **Then** results remain next to the imported file, not forced into B.
3. **Given** a transcription job was already queued for a file in A before the folder change, **When** that job runs, **Then** it still writes that job’s user-visible results beside the A file (the job’s source), while the next *new* recording uses B.
...
- **FR-006**: Imported files (picker or drop) MUST keep user-visible transcripts next to the source file; they MUST NOT be copied into Carpeta de salida solely because that folder was selected.
...
- Feature 009 (transcribe any file) remains: imports stay source-adjacent; this change does not redefine import layout.
